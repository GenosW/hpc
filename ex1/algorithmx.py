...
def algorithm_x(
    i: int, j: int, X: CommunicationStructure, debug: bool = False, verbose: bool = True
):
    if verbose:
        print(f"Computing path from i={i} -> j={j} in X_{X.d}")
    if i >= X.num_nodes or j >= X.num_nodes:
        raise ValueError(f"i or j too large! {i} or {j} >= {X.num_nodes}")
    if i == j:
        return [i]
    ...
    j_bits_r: str = binary(j)[
        ::-1
    ]  # reversing the order so it's b0 b1 b2 ... --> easier to work with
